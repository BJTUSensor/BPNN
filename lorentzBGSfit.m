function [yprime params resnorm residual] = lorentzBGSfit(x,y,p0,bounds,nparams)
% LORENTZBGSFIT fits a single- or multi-parameter Lorentzian function to data
%
%   LORENTZBGSFIT(X,Y) returns YPRIME(X), a Lorentzian fit to the data
%   found using LSQCURVEFIT. The function Y(X) is fit by the model:
%       YPRIME(X) = P1./((X - P2).^2 + P3) + C.
%
%   [YPRIME PARAMS RESNORM RESIDUAL] = LORENTZBGSFIT(X,Y) returns YPRIME(X) 
...
%
%   [...] = LORENTZBGSFIT(X,Y,P0) can be used to provide starting
%   values (P0 = [P01 P02 P03 C0]) for the parameters in PARAMS.
%
%   [...] = LORENTZBGSFIT(X,Y,P0,BOUNDS) may be used to define lower 
...
%
%   [...] = LORENTZBGSFIT(X,Y,P0,BOUNDS,NPARAMS) may be used to specify the
...
%       [yprime1 params1 resnorm1 residual1] = LORENTZBGSFIT(x,y,[20 10 15 0]);
%       figure; plot(x,y,'b.','LineWidth',2)
%       hold on; plot(x,yprime1,'r-','LineWidth',2)
%
%       [yprime2 params2 resnorm2 residual2] = LORENTZBGSFIT(x,y,[],[],'3');
%       figure; plot(x,y,'b.','LineWidth',2)
...
% lb = [-Inf,-Inf,-Inf,-Inf];
% ub = [Inf,Inf,Inf,Inf];
% lb = [0,min(x(:)),0,0,min(x(:)),0,-max(y(:))];
% ub = [Inf,max(x(:)),1,Inf,max(x(:)),1,max(y(:))];
lb = [-0.5,   min(x(:)), 0, 0];
ub = [1.2, max(x(:)), (max(x(:))-min(x(:))).^2, 1];
